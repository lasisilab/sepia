...
The substrate for Lily's coverage/variability analyses (#1–#3), **repaired**: the thesis
figures measured the 222 panel SNPs at their **hg38** positions in **hg19** data (~292 kb
off-target — bug A2). These matrices read the same panel at the **correct hg19 coordinates**
and span **moderns + archaics**, not archaics alone.

## Files
- `panel_hg19_genotypes.tsv` — base-level genotype (`%TGT`, e.g. `G/A`, `G/G`, `./.` for
  missing) per sample × locus. 161 variable panel loci × 164 samples.
...
`scratch/pig_panel/pig_panel.vcf.gz` (the B2a extraction,
[`code/pig_panel_extract.slurm`](../../code/pig_panel_extract.slurm)). Plotted by
[`plan/figures/make_panel_heatmaps.R`](../../plan/figures/make_panel_heatmaps.R) →
`plan/figures/panel_{atgc,depth}_heatmap.png`.
